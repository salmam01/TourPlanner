        ...
        public void ExportToursToExcel(IEnumerable<Tour> tours, string filePath)
        {
            using (XLWorkbook workbook = new XLWorkbook())
            {
                ...
                foreach (Tour tour in tourList)
                {
                    ...
                    toursSheet.Cell(tourRow, 7).Value = tour.TourDetails?.Distance ?? 0;
                    toursSheet.Cell(tourRow, 8).Value = tour.TourDetails?.EstimatedTime.ToString() ?? "00:00:00";
            ...
            using (XLWorkbook workbook = new XLWorkbook(filePath))
            {
                ...
                foreach (var row in toursSheet.RowsUsed().Skip(1)) // skip header
                {
                    ...
                    var tour = new Tour
                    {
                        ...
                        {
                            Distance = row.Cell(7).GetDouble(),
                            EstimatedTime = TimeSpan.TryParse(row.Cell(8).GetString(), out var est) ? est : TimeSpan.Zero
                        },
